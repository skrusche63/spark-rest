...
class AdminMaster extends BaseActor {
  ...
  protected def actor(worker:String):ActorRef = {
    ...
    worker match {
      ...
      case "fields" => context.actorOf(Props(new FieldWorker()))
      case "params" => context.actorOf(Props(new ParamWorker()))
      
      case "register" => context.actorOf(Props(new RegisterWorker()))
      case "status"   => context.actorOf(Props(new StatusWorker()))
